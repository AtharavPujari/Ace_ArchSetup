        ...
        if activews then
            ...
            if action == "move" then
                ...
                if win and win.address then
                    return hl.dispatch(hl.dsp.window.move({ window = "address:" .. win.address, workspace = z, follow = (follow == nil or follow == true) }))
                end
            else
...
local function move_active_window(target_workspace, follow)
    return function()
        local win = hl.get_active_window()
        if win and win.address then
            return hl.dispatch(hl.dsp.window.move({ window = "address:" .. win.address, workspace = target_workspace, follow = (follow == nil or follow == true) }))
        end
    end
end
